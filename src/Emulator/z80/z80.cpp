...
#define Z80ReadMem(where) (mappedMemoryData[(where) >> 14][(where) & 0x3FFF])
#define Z80WriteMem(where, A, regs) ({              \
  int memoryBank = (where) >> 14;                   \
  if (memoryBank != 0)                              \
  {                                                 \
    mappedMemoryData[memoryBank][(where) & 0x3fff] = A; \
...
uint16_t Z80Run(Z80Regs *regs, int numcycles)
{
  ...
  uint8_t **mappedMemoryData = memory.mappedMemoryData;
  bool **mappedMemoryIsDirty = memory.mappedMemoryIsDirty;
