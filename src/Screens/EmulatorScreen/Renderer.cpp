...
void Renderer::drawSpectrumScreen() {
  ...
  for (int attrY = 0; attrY < 192 / 8; attrY++)
  {
    ...
    for (int attrX = 0; attrX < 256 / 8; attrX++)
    {
      ...
      // Calculate the base offset for the first row of pixels for this attribute block
      // In the ZX Spectrum, the 8 rows of pixels for a character block are exactly 256 bytes apart.
      int baseOffset = ((attrY & 0x18) << 8) | ((attrY & 0x07) << 5) | attrX;
      uint8_t *currPixelPtr = pixelBase + baseOffset;
      uint8_t *copyPixelPtr = pixelBaseCopy + baseOffset;
      uint16_t *pixelAddress = pixelBuffer + attrX * 8;

      for (int y = 0; y < 8; y++)
      {
        // read the value of the pixels
        uint8_t row = *currPixelPtr;
        uint8_t rowCopy = *copyPixelPtr;
