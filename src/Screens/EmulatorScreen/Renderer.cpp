  ...
  for (int attrY = 0; attrY < 192 / 8; attrY++)
  {
    ...
    for (int attrX = 0; attrX < 256 / 8; attrX++)
    {
      ...
      for (int y = 0; y < 8; y++)
      {
        ...
        uint16_t *pixelAddress = pixelBuffers[currentPixelBuffer] + 256 * y + attrX * 8;
    ...
    if (dirty || firstDraw)
    {
      if (!isShowingMenu || borderHeight + attrY * 8 < m_tft.height() - VOLUME_BAR_HEIGHT) {
        m_tft.dmaWait(); // wait for previous DMA transfer to complete
        m_tft.setWindow(borderWidth, borderHeight + attrY * 8, borderWidth + 255, borderHeight + attrY * 8 + 7);
        m_tft.pushPixelsDMA(pixelBuffers[currentPixelBuffer], 256 * 8);
        currentPixelBuffer = 1 - currentPixelBuffer; // swap buffers
      }
    }
  }
  drawReady = true;
