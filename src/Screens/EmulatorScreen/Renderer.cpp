...
void Renderer::drawSpectrumScreen() {
  ...
  for (int attrY = 0; attrY < 192 / 8; attrY++)
  {
    ...
    // Pass 1: Check for dirtiness and update shadow buffers
    ...
    // Pass 2: Decode and push to TFT only if the row is dirty or it's the first draw
    if (dirty || firstDraw)
    {
      for (int attrX = 0; attrX < 256 / 8; attrX++)
      {
        // use the updated attribute from the shadow buffer
        ...
        {
          int scan = (screenY & B11000000) + (y << 3) + ((screenY & B111000) >> 3);
          uint8_t row = *(pixelBaseCopy + 32 * scan + attrX); // use the shadow buffer
