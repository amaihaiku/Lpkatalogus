  ...
  for (int attrY = 0; attrY < 192 / 8; attrY++)
  {
    ...
    for (int attrX = 0; attrX < 256 / 8; attrX++)
    {
      ...
      for (int y = 0; y < 8; y++)
      {
        ...
        uint16_t *pixelAddress = pixelBuffer[currentBuffer] + 256 * y + attrX * 8;
    ...
    if (dirty || firstDraw)
    {
      if (!isShowingMenu || borderHeight + attrY * 8 < m_tft.height() - VOLUME_BAR_HEIGHT) {
        m_tft.dmaWait(); // Ensure previous DMA transfer is complete
        m_tft.setWindow(borderWidth, borderHeight + attrY * 8, borderWidth + 255, borderHeight + attrY * 8 + 7);
        m_tft.pushPixelsDMA(pixelBuffer[currentBuffer], 256 * 8);
        currentBuffer = 1 - currentBuffer; // Switch buffer
      }
    }
  }
  drawReady = true;
