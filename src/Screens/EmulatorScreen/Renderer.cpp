...
void Renderer::drawSpectrumScreen() {
  ...
  for (int attrY = 0; attrY < 192 / 8; attrY++)
  {
    ...
    // First pass: check for any dirty attribute or pixel in the whole 8-pixel-high row (32 attribute blocks wide)
    if (!firstDraw) {
      if (memcmp(attrBase + 32 * attrY, attrBaseCopy + 32 * attrY, 32) != 0) {
        ...
        // Flashing attributes can cause a redraw even if the attr bytes didn't change
        // We only check if the row has flashing attributes
        if (!dirty) {
        ...
        *(attrBaseCopy + 32 * attrY + attrX) = attr;
        ...
        for (int y = 0; y < 8; y++)
        {
          ...
          *(pixelBaseCopy + 32 * scan + attrX) = row;
          ...
          if (row == 0) {
            uint32_t u32Clr = (uint32_t)(tftPaperColor | (tftPaperColor << 16));
            ...
            *d32++ = u32Clr;
            ...
            uint32_t u32Clr = (uint32_t)(tftInkColor | (tftInkColor << 16));
            uint32_t *d32 = (uint32_t *)pixelAddress;
            *d32++ = u32Clr;
            *d32++ = u32Clr;
            *d32++ = u32Clr;
