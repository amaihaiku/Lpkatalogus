...
void Renderer::drawSpectrumScreen() {
  ...
  for (int attrY = 0; attrY < 192 / 8; attrY++)
  {
    ...
    // Fast check to see if this entire character row (8 pixels high, 32 chars wide) has changed
    // We check attributes first. 32 bytes per row.
    if (memcmp(attrBase + 32 * attrY, attrBaseCopy + 32 * attrY, 32) != 0) {
      dirty = true;
    } else if (!firstDraw && flashTimer < 16) {
       // Check if any attributes in this row are flashing
       for (int attrX = 0; attrX < 32; attrX++) {
         if ((*(attrBase + 32 * attrY + attrX) & B10000000) != 0) {
            dirty = true;
            break;
         }
       }
    ...
    // Precalculate scanline offsets for this character row
    int screenY = attrY * 8;
    int scan[8];
    for (int y = 0; y < 8; y++) {
    ...
    if (!dirty && !firstDraw) {
      // Attributes are clean and not flashing. Check pixel rows.
      for (int y = 0; y < 8; y++) {
